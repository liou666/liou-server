...
const notFoundTemplate = handleBars.compile(require("./template/404_template"))
...
module.exports = class Server {
    constructor(options = {}) {
        ...
        this.defaultShowPage = options.defaultShowPage
        this.cors = options.cors
        this.maxAge = options.maxAge
        this.charset = options.charset
        this.isOpenDefault = options.openDefault
     ...
     * @param {object} stat 
     */
    async responseFile(req, res, pathName) {
        const stats= await stat(process.cwd() + pathName)

        res.setHeader('Content-Type', (mime.getType(pathName) || 'application/json') + `;charset=${this.charset}`)
        ...
        readStream.on('end', async () => {
            ...
            let rs = await this.getReadStream(req, res, pathName, stats)
    ...
    async handleRoute(req, res, pathName) {
        ...
        if (stats.isDirectory()) {
            ...
            files.includes(this.defaultShowPage) && this.isOpenDefault
                ? this.responseFile(req, res, path.join(pathName, this.defaultShowPage))
                : this.responseDirectory(res, pathName, files)
        } else { //文件操作
            this.responseFile(req, res, pathName)
        }
    }
